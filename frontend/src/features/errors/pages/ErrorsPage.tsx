	...
	const items: ErrorPanelState[] = [
		{
			...
			value: validationModel.error ??
				validationModel.data ?? { message: "No response" },
		...
		},
		{
			...
			value: domainException.error ??
				domainException.data ?? { message: "No response" },
			isLoading: domainException.isLoading,
		},
		{
			label: "GetUnauthorizedError",
			value: unauthorized.error ??
				unauthorized.data ?? { message: "No response" },
		...
		},
		{
			...
			value: internalServerError.error ??
				internalServerError.data ?? { message: "No response" },
		...
		<div className="space-y-6 p-6">
			<div className="space-y-2">
				...
				<p className="text-sm opacity-70">
					This page intentionally calls demo endpoints that return problem
					details.
				</p>
			</div>
			...
			<div className="grid gap-4">
				{items.map((item) => (
					...
					>
						...
						<pre className="overflow-auto rounded-box bg-base-200 p-4 text-sm mt-3">
							{item.isLoading
								? "Loading..."
