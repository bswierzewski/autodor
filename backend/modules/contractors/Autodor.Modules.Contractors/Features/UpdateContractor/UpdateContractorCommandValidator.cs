...
{
    public UpdateContractorCommandValidator(ContractorsDbContext dbContext)
    {
        RuleFor(x => x.NIP)
            ...
            .MustAsync(async (command, nip, ct) => !await dbContext.Contractors.AnyAsync(c => c.NIP == new TaxId(nip) && c.Id != new ContractorId(command.Id), ct))
